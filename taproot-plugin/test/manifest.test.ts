...
test('extension manifest contributes a Taproot activity bar view', async () => {
  ...
  assert(!extensionSource.includes("status.text = '$(person) Taproot';"));
  assert(!extensionSource.includes("status.text = '$(remote) Taproot';"));
